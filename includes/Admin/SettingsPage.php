...
function wca_enqueue_admin_assets($hook) {
    if ($hook !== 'woocommerce_page_' . WCA_MENU_SLUG) return;
    ...
    wp_localize_script('wca-admin', 'WCA_PRESETS', array(
        'presets'  => $presets,
        'opts'     => $opts,
        'ajax'     => array(
...
function wca_sanitize_opts_ext($input) {
    ...
    $def = wca_defaults(); $out = array();

    // checkboxes
    $checkboxes = array(
        'enabled','enable_logging','use_honeypots','use_timestamp','strict_ref',
        'block_disposable_email','block_if_only_flagged','require_login_below',
        'block_guest_for_flagged','enable_device_age','validate_phone','validate_postal',
        'enable_reject_keywords','enable_gateway_friction'
    );
    foreach ($checkboxes as $k) { $out[$k] = isset($in[$k]) ? 1 : 0; }
    ...
    foreach (array('allow_countries','deny_countries','flag_product_ids','block_message','phone_regex','postal_regex') as $k) {
        if (!isset($out[$k])) $out[$k] = isset($in[$k]) ? sanitize_text_field($in[$k]) : (string)($def[$k] ?? '');
    }
    ...
    <div class="wrap wca-wrap">
        ...
        <form method="post" action="options.php" class="wca-form">
            ...
            <?php if (!empty($schema[$tab]['fields'])): ?>
                <div class="wca-grid">
                    <?php foreach ($schema[$tab]['fields'] as $f):
                        $id=$f[0]; $type=$f[1]; $label=$f[2]; $def=$f[3]; $help=$f[4] ?? '';
...
function wca_render_input($id, $type, $val, $help) {
    ...
    switch ($type) {
        ...
        case 'select':
            $choices = array();
            if ($id==='flag_match_mode') $choices = array('any'=>__('Any (OR)','wc-anti-fraud-pro-lite'),'all'=>__('All (AND)','wc-anti-fraud-pro-lite'));
        ...
        case 'product_multi':
            ...
            foreach ($ids as $pid) {
                ...
                if ($title) $selected[$pid] = $title . ' (#'.$pid.')';
            }
    ...
    if (!empty($help)) {
        if (is_array($help)) $help = implode(' ', $help);
...
function wca_handle_tools() {
    ...
    if (!isset($_POST['wca_tool'])) return;
    if (!isset($_POST['wca_tools_nonce']) || !wp_verify_nonce($_POST['wca_tools_nonce'], 'wca_tools_nonce')) return;
...
function wca_export_settings() {
    ...
    if (!isset($_POST['wca_export_nonce']) || !wp_verify_nonce($_POST['wca_export_nonce'], 'wca_export_nonce')) wp_die('');
    $opts = wca_opt();
...
function wca_import_settings() {
    if (!current_user_can('manage_woocommerce')) wp_die('');
    if (!isset($_POST['wca_import_nonce']) || !wp_verify_nonce($_POST['wca_import_nonce'], 'wca_import_nonce')) wp_die('');
    if (!isset($_FILES['wca_file']) || empty($_FILES['wca_file']['tmp_name'])) wp_die('');
...
function wca_on_settings_updated($old, $new) {
    ...
    $changed = array();
    foreach ($new as $k => $v) { if (!array_key_exists($k,$old) || $old[$k] !== $v) $changed[] = $k; }
    if ($changed) wca_log_event('settings_updated', array('keys'=>$changed), 'info');
}