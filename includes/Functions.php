...
function wca_lines_to_array($text) {
    $out = array();
    foreach (explode("\n", (string)$text) as $line) { $line = trim($line); if ($line !== '') $out[] = $line; }
    return $out;
}
function wca_csv_to_array($csv) {
    $out = array();
    foreach (explode(',', (string)$csv) as $p) { $p = trim($p); if ($p !== '') $out[] = $p; }
    return $out;
}
...
function wca_request_id() {
    static $rid = null;
    if ($rid === null) { $rid = substr(bin2hex(random_bytes(8)), 0, 12); }
    return $rid;
}
...
function wca_redact_ip($ip) {
    if (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
        $parts = explode('.', $ip); $parts[3] = 'x'; return implode('.', $parts);
    }
...
function wca_common_ctx() {
    ...
    if (function_exists('WC') && WC()->cart) {
        ...
        foreach (WC()->cart->get_cart() as $it) {
            $items[] = array('pid'=>(int)($it['product_id']??0),'qty'=>(int)($it['quantity']??1));
        }
    }
    return array(
        'rid'=>wca_request_id(),'ip'=>$ip?wca_redact_ip($ip):'','ua'=>$ua,'ref'=>$ref,
        'uid'=>$uid? (int)$uid : 0,'guest'=>$uid?0:1,'total'=>$total,'items'=>$items,'time'=>time()
    );
}
function wca_log_event($event, array $data=array(), $level='info') {
    if (!function_exists('wc_get_logger')) return;
    $o = wca_opt(); if (empty($o['enable_logging'])) return;
    if (isset($data['email']))    $data['email']   = wca_redact_email($data['email']);
    if (isset($data['ip']))       $data['ip']      = wca_redact_ip($data['ip']);
    if (isset($data['order_id'])) $data['order_id']= (int)$data['order_id'];
    $payload = array_merge(array('event'=>(string)$event), wca_common_ctx(), $data);
    $json = wp_json_encode($payload, JSON_UNESCAPED_SLASHES);
    if (strlen($json) > 8000) $json = substr($json,0,8000).'...';
...
function wca_email_in_list($email, $list_text) {
    ...
    foreach (wca_lines_to_array($list_text) as $line) {
        ...
        if ($line === $email || ($domain && $line === $domain)) return true;
    }
    return false;
}
...
function wca_bad_ua($ua) {
    ...
    foreach (wca_lines_to_array(wca_opt('ua_blacklist')) as $needle) {
        if ($needle !== '' && strpos($ua, strtolower($needle)) !== false) return true;
    }
    return false;
}
...
function wca_is_banned($ip) { return (bool) get_transient('wca_ban_'.md5($ip)); }
...
function wca_list_bans() {
    ...
    foreach ($rows as $r) {
        ...
        $out[] = array('hash'=>$hash,'seconds_left'=>$left);
    }
    return $out;
}

/* ----------- Validation presets ----------- */
function wca_validation_presets() {
    // Presets use PHP regex with delimiters; examples power the live preview
    return array(
        'generic' => array(
            'label'  => 'Generic',
            'phone'  => '/^\+?[0-9\s().-]{7,20}$/',
            ...
            'examples' => array(
                'phone'  => array('good'=>array('+1 415 555 2671','07123 456789'),'bad'=>array('abc-123','123')),
                'postal' => array('good'=>array('12345','AB12 3CD'),'bad'=>array('***','1'))
            ),
        ),
        'us' => array(
            'label'=>'United States',
            ...
            'postal'=>'/^\d{5}(-\d{4})?$/',
            'examples'=>array(
                'phone'=>array('good'=>array('(415) 555-2671','415-555-2671'),'bad'=>array('07123 456789','1234')),
                'postal'=>array('good'=>array('94107','10001-1234'),'bad'=>array('AB12 3CD'))
            ),
        ),
        'uk' => array(
            ...
            'examples'=>array(
                'phone'=>array('good'=>array('07123 456789','+44 20 7123 4567'),'bad'=>array('415-555-2671','123456')),
                'postal'=>array('good'=>array('SW1A 1AA','EC1A 1BB'),'bad'=>array('94107','AAA 1'))
            ),
        ),
        'ca' => array(
            'label'=>'Canada',
            ...
            'examples'=>array(
                'phone'=>array('good'=>array('416-555-2671'), 'bad'=>array('07123 456789')),
                'postal'=>array('good'=>array('M5V 2T6'), 'bad'=>array('94107'))
            ),
        ),
        'au' => array(
            'label'=>'Australia',
            'phone'=>'/^(\+?61\s?)?(\(0\))?\s?\d{1,2}\s?\d{4}\s?\d{4}$/',
            'postal'=>'/^\d{4}$/',
            'examples'=>array(
                'phone'=>array('good'=>array('+61 2 9876 5432'), 'bad'=>array('07123 456789')),
                'postal'=>array('good'=>array('2000'), 'bad'=>array('SW1A 1AA'))
            ),
        ),
        'eu' => array(
            'label'=>'EU (common formats)',
            'phone'=>'/^\+?\d[\d\s().-]{6,19}$/',
            'postal'=>'/^[A-Za-z0-9\s-]{3,10}$/',
            'examples'=>array(
                'phone'=>array('good'=>array('+33 1 23 45 67 89'), 'bad'=>array('abc-123')),
                'postal'=>array('good'=>array('75001','10115','00144'), 'bad'=>array('**'))
            ),
        ),
    );
}
...
function wca_profile_for_country($country) {
    ...
    if ($c === 'GB' || $c === 'UK' || $c === 'IM' ) return 'uk';
    if ($c === 'US') return 'us';
    if ($c === 'CA') return 'ca';
    if ($c === 'AU' || $c === 'NZ') return 'au';
    if (in_array($c, array('FR','DE','IT','ES','PT','NL','BE','AT','SE','DK','FI','IE','PL','CZ','SK','HU','RO','GR','BG','HR','SI','EE','LV','LT','LU','MT','CY'), true)) return 'eu';
    return 'generic';
}