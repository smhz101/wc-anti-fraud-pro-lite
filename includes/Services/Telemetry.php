...
function wca_on_create_order($order, $data) {
    ...
    if ($ip) $order->set_customer_ip_address($ip);
    if ($ua) $order->update_meta_data('_customer_user_agent', $ua);
    $order->update_meta_data('_customer_referrer', $ref ?: 'None');

    wca_log_event('order_created', array('order_id'=>(int)$order->get_id(),'ip'=>$ip), 'info');
}
