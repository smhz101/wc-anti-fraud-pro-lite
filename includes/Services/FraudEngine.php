...
function wca_checkout_fields() {
	$o = wca_opt();
	if ( empty( $o['enabled'] ) || empty( $o['use_honeypots'] ) ) {
		return;
	}
	if ( ! function_exists( 'WC' ) || ! WC()->session ) {
		return;
...
function wca_validate_checkout( $data, $errors ) {
	...
	if ( ! empty( $o['enable_device_age'] ) ) {
		...
		if ( ! $checks['device_age_ok'] ) {
			$block     = true;
			$reasons[] = 'device_too_new'; }
	}
	...
	if ( $email && ! wca_email_in_list( $email, $o['email_whitelist'] ) ) {
		if ( $email && wca_email_in_list( $email, $o['email_blacklist'] ) ) {
			$block     = true;
			$reasons[] = 'email_blacklist'; }
		if ( ! empty( $o['block_disposable_email'] ) && $email && wca_is_disposable( $email ) ) {
			$block     = true;
	...
	if ( $country ) {
		...
		if ( ! empty( $allow ) && ! in_array( $country, $allow, true ) ) {
			$block     = true;
			$reasons[] = 'country_not_allowed'; }
		if ( ! empty( $deny ) && in_array( $country, $deny, true ) ) {
			$block     = true;
			$reasons[] = 'country_denied'; }
	}
	...
	if ( ! empty( $o['enable_reject_keywords'] ) ) {
		...
		foreach ( wca_lines_to_array( $o['reject_address_keywords'] ) as $kw ) {
			$kw = strtolower( $kw );
			if ( $kw !== '' && ( strpos( $addr1, $kw ) !== false || strpos( $addr2, $kw ) !== false ) ) {
				$block     = true;
				$reasons[] = 'addr_keyword:' . $kw;
				break; }
		}
	}
